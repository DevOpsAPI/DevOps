...
app = FastAPI()
default_message = "Hoi! Welcome to the Game API!"
...
def create_genre(genre: Genre, db: Session = Depends(get_db)):
    db_genre = models.Genre(genre_id=genre.genre_id,genre_name=genre.genre_name)
    db.add(db_genre)
    db.commit()
    db.refresh(db_genre)
    return db_genre
...
def create_game_publisher(game_publisher: Game_Publisher, db: Session = Depends(get_db)):
    db_game_publisher = models.Game_Publisher(game_publisher_id=game_publisher.game_publisher_id,game_publisher_name=game_publisher.game_publisher_name)
